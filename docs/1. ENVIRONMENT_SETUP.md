...
### ✅ 필수
- [.NET 10 SDK](https://dotnet.microsoft.com/download/dotnet/10.0) (**필수**)
- [Visual Studio 2022](https://visualstudio.microsoft.com/) (17.12 이상 권장) 또는 [VS Code](https://code.visualstudio.com/)
...
> ⚠️ **주의**: .NET 10 지원을 위해 Visual Studio 2022 **17.12 이상** 버전이 필요합니다.
...
- `supabase` - Supabase Python 클라이언트 (유사물건 추천용)
...
Storage → New Bucket에서 다음 버킷들을 생성:

1. **registry-pdfs** (등기부등본 PDF 저장)
   - Public: No (비공개)
   
2. **excel-files** (엑셀 파일 저장)
   - Public: No (비공개)
...
- [ ] .NET 10 SDK 설치 확인 (`dotnet --version` → 10.0.x)
- [ ] Visual Studio 2022 17.12+ 또는 VS Code 설치
- [ ] Python 3.10+ 설치 확인 (`python --version`)
- [ ] Python 패키지 설치 완료 (`pip list`)
- [ ] Tesseract OCR 설치 확인
- [ ] Supabase 프로젝트 생성
- [ ] API 키 확보 및 `appsettings.json` 설정
- [ ] Storage 버킷 생성 (`registry-pdfs`, `excel-files`)
...
| ClosedXML | 0.102.3 | Excel 처리 |
| supabase-csharp | 0.16.2 | Supabase 연동 |
| Microsoft.Web.WebView2 | 1.0.3595.46 | 지도 표시 |
| LiveChartsCore.SkiaSharpView.WPF | 2.0.0-rc2 | 차트 |
| Serilog | 3.1.1 | 로깅 |
