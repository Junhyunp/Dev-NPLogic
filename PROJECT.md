...
- 유형별 실거래가 표시 분기: 아파트=총거래가(만원), 연립다세대/공장창고/상가아파트형공장=전용면적당 단가(만원/㎡), 주택/토지/기타=실거래가 비표시
- 실거래가 UI 동적 처리: 섹션 헤더 동적 바인딩(CaseMapSectionTitle), 우측 패널 visibility(IsShowRealTransaction), 금액 컬럼 헤더 동적 변경(AmountColumnHeader)
- 낙찰통계 테이블 개편: 10열→8열 (지역별 가율/건수 2열씩 + 적용낙찰가율 열 추가), 데이터 셀 ViewModel 바인딩 완료
- 적용낙찰가율 열 신설: Row0 PrimaryBrush 헤더, Row1 설명 텍스트(AppliedBidRateDescription), Row2-4 RowSpan=3 편집가능 TextBox + 고정 "%" 표시
- 낙찰통계 하드코딩 기본값 제거: 지역명/적용낙찰가율/설명 null 초기화 (경매DB 연동 시 자동 산출 예정)

### 2026-03-11

- 사례지도 및 실거래가 섹션 레이아웃 개편: 사례지도를 전체 높이로 확장, 우측에 실거래가 표 + 거래가격/건수 그래프 수직 배치 (기존 2열 차트 플레이스홀더 → 단일 통합 그래프 영역)
- 실거래가 면적 필터링: 아파트 물건의 BuildingArea(전용면적) 기준 동일면적 1년치 최근순 우선 표시 + 유사면적 폴백
- 거래가격/건수 이중축 차트 구현 (LiveCharts2): 면적 드롭다운 선택, 막대=건수(우축), 꺾은선=평균금액(좌축, 억 단위 F2), 연도별 X축, 가로 그리드선 자동 스케일

### 2026-03-10

- 경(공)매일정 탭 선택 스타일 개선 (선택 탭: PrimaryBrush 배경 + 흰색 텍스트, 비선택 탭: 투명 배경 + 회색 텍스트, 호버 시에도 선택 탭 색상 유지)

### 2026-03-05

- 공장/창고 전용 탐문 결과 섹션 신설 (+/− 테이블 생성·제거, 5열 고정 4행: 토지·건물·기계·합계)
- 공장/창고 전용 탐문 수익가치 Optional 섹션 신설 (+/− 테이블 생성·제거, 4x9 좌우 병렬 표: 건물면적/평당임대료/월임대료/연임대료/보증금 + 기본할인율/민감도 기준/수익가치/평당가액 5% 구간)
...
- 공장/창고 전용 현재 임대 수익가치 Optional 섹션 신설 (+/− 테이블 생성·제거, 상단 DataGrid 9열: 구분/면적/임차인/임차개시일/임차종료일/확정신고일/보증금/월세/연임대료 + 하단 3x9 민감도 분석: 기본할인율/민감도기준/수익가치/평당가액 5% 구간)

### 2026-03-04

- 공장/창고 전용: 사례평가 컬럼 44행 (기계기구/제시외/기계인정율/공시지가 등), 사례 로드뷰 4x2 섹션, 탐문 내역 섹션
- 사례평가 컬럼 유형별 분기 (SetProperty 시 InitializeCaseItems 재호출)
