...
ref_dir = r'c:\Users\pwm89\dev\nplogic\manager\ref'
output_dir = os.path.join(ref_dir, 'csv_output')
os.makedirs(output_dir, exist_ok=True)

excel_files = [f for f in os.listdir(ref_dir) if f.endswith('.xlsx')]
print(f'찾은 엑셀 파일: {len(excel_files)}개')

for excel_file in excel_files:
    excel_path = os.path.join(ref_dir, excel_file)
    print(f'\n처리 중: {excel_file}')
    ...
    for sheet_name in sheet_names:
        try:
            ...
            print(f'  [OK] {sheet_name} -> {csv_filename}')
